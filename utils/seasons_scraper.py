...
class SeasonsScraper:
    ...
    def scrape_event(self, gender, age_category, discipline_slug, type_slug, output_dir, year):
        page = 1
        data = []
        
        log_dir = os.path.join("logs/seasons", self.today)
    ...
    def run(self, year=None, max_workers=10):
        if year is None:
            year = self.current_year
            
        jobs = []
        completed_years = [] 
        print(f"Starting SEASONS scrape for {year} using {max_workers} workers...")
        start_time = time.time()

        queue_file = f"seasons/queues/queue_seasons_{year}.json"
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_job):
                ...
                try:
                    ...
                    # Queue Management on Success (For historical seasons)
                    if success:
                        if year != self.current_year:
                            jobs.remove(job)
                            with open(queue_file, "w") as f:
