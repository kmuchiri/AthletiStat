...
'''
# --- SCRAPER ---
def scrape_event(gender, age_category, discipline_slug, type_slug, output_dir, year=current_year):
    page = 1
    data = []
    ...
    while True:
        ...
        try:
            ...
            return False # Returns False so the queue doesn't remove the job
    ...
    if data:
        ...
        filename = f"{year}{type_slug}_{discipline_slug}_{age_category}.csv".replace(" ", "_").replace("/", "-")
...
def run_scraper(mappings, max_workers=10, year=None):
    jobs = []
    completed_years = [] 
    print(f"Starting Seasons scrape using {max_workers} workers...")
